...
def list(request,type_id,page):
    ...
    #分页 实例化分页对象 - 参数 - 分页对象 每页装几条数据
    paginator = Paginator(books_li,1)
    #获取分页之后的总页数 - num_pages分页实例的属性
    ...
    if page == '' or int(page) > num_pages:
        ...
        page = int(page)
    #分页实例属性page是获取第page页的数据
    books_li = paginator.page(page)
    #进行页码控制
    '''
    1.总页数<5,显示所有页码 eg:2   12
    2.当前页是前三页,显示1-5页 [1] 2345  1 [2] 345  12 [3] 45
    3.当前页是后三页,显示后五页 23 [4] 56
    4.其他情况显示前两页当前页后两页 34 [5] 67
    '''
    if num_pages < 5:
